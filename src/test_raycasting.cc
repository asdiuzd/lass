...
void test_raycasting_robotcar(int argc, char** argv) {
    ...
    for (int idx = 0; idx < es.size(); idx++) {
        ...
        switch (camera_type) {
        case 0:
            mm->raycasting_target_pcd(e, left_intrsinsics, pcd);
            break;
        
        case 1:
            mm->raycasting_target_pcd(e, rear_intrsinsics, pcd);
            break;
        
        case 2:
            mm->raycasting_target_pcd(e, right_intrsinsics, pcd);
            break;
...
void test_generate_files_robotcar(int argc, char** argv) {
    /*
        argv[1] -- info file
        argv[2] -- list file
        argv[3] -- dir
        argv[4] -- output centers json file name
        argv[5] -- output extrinsics json file name
     */
    Eigen::Matrix4f left, rear, right;
    Eigen::Matrix3f intrinsics;
    json j_es, j_label;
